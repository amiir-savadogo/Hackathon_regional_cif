...
const critTable = new Table({
  ...
  rows: [
    new TableRow({ children: [cell("Critère de sélection CIF", { header: true, width: 5200, shade: BLUE }), cell("Réponse de Samdé", { header: true, width: 4150, shade: BLUE })] }),
...
const compTable = new Table({
  ...
  rows: [
    new TableRow({ children: [cell("Approche", { header: true, width: 3117, shade: BLUE }), cell("Solutions génériques (Kaggle « German Credit », « Home Credit »)", { header: true, width: 3117, shade: BLUE }), cell("Samdé", { header: true, width: 3116, shade: GREEN })] }),
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      new Paragraph({ children: [new TextRun({ text: "Samdé", bold: true, size: 44, color: BLUE })], spacing: { after: 40 } }),
      ...
      p("Samdé est un prototype de scoring automatisé du risque de microcrédit, pensé dès la conception pour le contexte réel d'une Coopérative financière ouest-africaine plutôt que pour transposer un modèle bancaire occidental. Trois choix structurent l'approche :"),
      ...
      bullet("Dataset synthétique documenté (data/samde_dataset.csv) et script de génération reproductible.", false),
      ...
      p("Équipe Samdé — 4 membres (3 profils Master Data Science, 1 profil Génie logiciel) couvrant l'ensemble de la chaîne de valeur du prototype :"),
...
Packer.toBuffer(doc).then((buf) => {
  require("fs").writeFileSync("note_presentation_Samdé.docx", buf);
  console.log("OK");
});
