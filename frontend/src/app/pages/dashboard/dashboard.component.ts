  ...
  template: `
    <div>
      ...
      <nav class="flex items-center space-x-2 text-xs font-medium text-gray-500 mb-5 bg-white px-4 py-2.5 rounded-xl border border-gray-200/80 shadow-sm" aria-label="Breadcrumb">
        <span class="inline-flex items-center text-blue-700 font-semibold">
      ...
      <div class="bg-[#e5f3f1] border border-[#b9ded9] rounded-xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h3 class="font-semibold text-[#123b41]">Enregistrer un nouveau demandeur</h3>
          <p class="text-sm text-[#147c76] mt-1">Créez d'abord le profil du client, puis soumettez sa demande de crédit.</p>
        </div>
        <a routerLink="/clients/nouveau"
          class="bg-[#147c76] hover:bg-[#0e625e] text-white text-sm font-semibold px-5 py-2.5 rounded-lg transition-colors whitespace-nowrap self-start sm:self-auto">
          + Nouveau client
        </a>
      </div>
    </div>
